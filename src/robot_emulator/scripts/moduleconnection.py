...
def bootModule(port):
	...
	if bootresponse!='noresp':
			print 'Rebuilding my internal model for port {}'.format(port)
			#if the port has an IO, put that into the correct component
			parsestring = bootresponse.split('/')


			#figure out what the new boot response string will be
			#for now just the type - locomotion

			print parsestring[1]
			
			print parsestring[4].split(',')[0]
			icomponent.settype(parsestring[4].split(',')[0])
